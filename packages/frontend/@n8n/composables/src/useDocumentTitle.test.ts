...
describe('useDocumentTitle', () => {
	...
	it('should set the document title', () => {
		...
		set('Test Title');
		expect(document.title).toBe('Test Title - jWorkflows');
	});
	...
	it('should reset the document title', () => {
		...
		set('Test Title');
		reset();
		expect(document.title).toBe('Workflow Automation - jWorkflows');
	});

	it('should use the correct suffix for the release channel', () => {
		const { set } = useDocumentTitle({ releaseChannel: 'beta' });
		set('Test Title');
		expect(document.title).toBe('Test Title - jWorkflows[BETA]');
	});
	...
	describe('setDocumentTitle', () => {
		it('should set document title with IDLE status', () => {
			...
			expect(document.title).toBe('▶️ My Workflow - jWorkflows');
		});

		it('should set document title with EXECUTING status', () => {
			const { setDocumentTitle } = useDocumentTitle();
			setDocumentTitle('My Workflow', 'EXECUTING');
			expect(document.title).toBe('🔄 My Workflow - jWorkflows');
		});
		...
		it('should set document title with AI_BUILDING status', () => {
			...
			expect(document.title).toBe('[Building] My Workflow - jWorkflows');
		});

		it('should set document title with AI_DONE status', () => {
			const { setDocumentTitle } = useDocumentTitle();
			setDocumentTitle('My Workflow', 'AI_DONE');
			expect(document.title).toBe('[Done] My Workflow - jWorkflows');
		});
	});
